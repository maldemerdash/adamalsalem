...
set manager_approved = true,
    manager_approved_at = coalesce(manager_approved_at, confirmed_at, created_at)
where booking_type in ('external', 'special_external_package')
  and confirmed = true
...
begin
  ...
  where confirmed = false
    ...
    and booking_type not in ('external', 'special_external_package')
...
begin
  ...
  where s.suspended = false
    ...
    and (
      s.slot_type not in ('internal', 'home')
      or ((s.date + s.time::time) at time zone 'Asia/Riyadh') > now()
    )
    and not exists (
      select 1
...
drop function if exists public.create_appointment_booking(text, text, text, text, text, text, text, boolean, boolean, numeric, numeric, text);
...
begin
  ...
  normalized_gender := lower(trim(coalesce(p_gender, '')));
  if normalized_gender not in ('male', 'female') then raise exception 'INVALID_GENDER'; end if;
  ...
  if selected_type in ('internal', 'home')
    and ((target_slot.date + target_slot.time::time) at time zone 'Asia/Riyadh') <= now()
  then raise exception 'SLOT_NOT_AVAILABLE'; end if;
  ...
  insert into public.appointment_bookings (
    slot_id, name, phone, gender, city, booking_type, region, visit_city,
    ...
    target_slot.id, normalized_name, normalized_phone, normalized_gender, selected_region,
    ...
    false, false,
    case
      when selected_type in ('external', 'special_external_package') then null
  ...
  return query select
    created_number, selected_type, normalized_gender, selected_region, selected_city, selected_price,
...
begin
  ...
  where b.phone = regexp_replace(coalesce(p_phone, ''), '\D', '', 'g')
    and (b.receipt_sent = true or b.confirmed = true)
...
begin
  ...
  where b.phone = regexp_replace(coalesce(p_phone, ''), '\D', '', 'g')
    ...
    and (
      b.booking_type not in ('external', 'special_external_package')
      or b.manager_approved = true
    );
...
returns table (
  id uuid, booking_number text, name text, phone text, gender text, city text,
  ...
  receipt_sent_at timestamptz, manager_approved boolean,
  manager_approved_at timestamptz, confirmed boolean,
...
begin
  ...
  return query
  select
    b.id, b.booking_number, b.name, b.phone, b.gender, b.city, b.booking_type,
    b.region, b.visit_city, b.visit_price, b.price_accepted_at,
    b.home_session, b.receipt_sent, b.receipt_sent_at,
    b.manager_approved, b.manager_approved_at,
    b.confirmed, b.attended, b.expires_at,
  ...
  where b.phone = regexp_replace(coalesce(p_phone, ''), '\D', '', 'g')
    and (
      nullif(trim(coalesce(p_booking_number, '')), '') is null
      or b.booking_number = trim(p_booking_number)
    )
...
drop function if exists public.track_appointment_booking(text);
drop function if exists public.track_appointment_booking_by_phone(text);
create function public.track_appointment_booking_by_phone(p_phone text)
returns table (
  booking_number text,
  booking_type text,
  region text,
...
begin
  ...
  select
    case
      when b.receipt_sent = true or b.confirmed = true or b.attended = true
        then b.booking_number
      else null
    end,
    b.booking_type,
    b.region,
...
revoke all on function public.create_appointment_booking(text, text, text, text, text, text, text, boolean, boolean, numeric, numeric, text) from public;
grant execute on function public.create_appointment_booking(text, text, text, text, text, text, text, boolean, boolean, numeric, numeric, text) to anon, authenticated;
...
revoke all on function public.track_appointment_booking_by_phone(text) from public;
grant execute on function public.track_appointment_booking_by_phone(text) to anon, authenticated;
